...
public class SleighLanguage implements Language {
	...
	private List<LanguageDecompileOption> decompileOptions = new ArrayList<>();
	...
	private void read(XmlPullParser parser) throws XmlParseException {
		...
		while (parser.peek().isStart()) {
			...
			else if (elName.equals("optionslist")) {
				XmlElement subel = parser.start();
				while (!parser.peek().isEnd()) {
					XmlElement option = parser.start();

					decompileOptions.add(new LanguageDecompileOption(option.getName(),
						!parser.peek().isEnd() ? parser.end(parser.start("param1")).getText() : "",
						!parser.peek().isEnd() ? parser.end(parser.start("param2")).getText() : "",
						!parser.peek().isEnd() ? parser.end(parser.start("param3")).getText() : ""));
	...
	@Override
	public List<LanguageDecompileOption> getDecompileOptions() {
		return Collections.unmodifiableList(decompileOptions);
	}
